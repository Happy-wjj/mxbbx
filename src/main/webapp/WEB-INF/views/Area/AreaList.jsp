...
<body>
					...
					<section class="vbox">
						<section class="scrollable padder">
							...
							<section class="panel panel-default">
								<header class="panel-heading">行政区域管理列表</header>

								<form action="<%= basePath %>area/list" method="post"
									...
									<div class="row text-sm wrapper">
										...
										<div class="col-sm-10 m-b-xs">
											<div class="inline" style="margin-right:20px;">
												所属区域编码 <input type="text" id="cityno" name="cityno"
													class="input-sm form-control inline" style="width: 150px;"
													placeholder="请输入行政编码" data-rangelength="[4,30]">

												<button class="btn btn-sm btn-default">查询</button>
											</div>

											<a href="<%= basePath %>area/toAdd"
													...
													<c:when test="${!empty page.list}">
														<c:forEach items="${page.list}" var="area">
															<tr>
																...
																<td style="line-height: 30px;text-align: center;">${area.cityno}</td>
																<td style="line-height: 30px;text-align: center;">${area.areaname}</td>
																<td style="line-height: 30px;text-align: center;">${area.level}</td>
																<td style="line-height: 30px;text-align: center;">
																	<a href="<%= basePath %>area/toEdit?id=${area.id}"
																	class="btn btn-warning btn-sm">修改</a> <a
																	href="<%= basePath %>area/delete?id=${area.id}"
	...
	<script type="text/javascript">
			...
			function getRequestURL(page) {
				...
				var url = host + "area/list?pageNo=" + pageNo + "&pageCount=" + pageCount 
							+ "&keyword=" + keyword;
				return url;
			}
