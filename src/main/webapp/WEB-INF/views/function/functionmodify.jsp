					...
					<section class="vbox">
						...
						<section class="scrollable padder">
							...
							<ul class="breadcrumb no-border no-radius b-b b-light pull-in">
								...
								<li><a href="<%= basePath %>fun/funs"> 菜单管理 </a></li>
								<li class="active">新增菜单</li>
							</ul>

							<!-- 表格标题 -->
							<div class="m-b-md">
								<h2 class="m-b-none">修改菜单</h2>
							</div>

							<form id="form" action="<%= basePath %>fun/modifyFunction"
								...
								<section class="panel panel-default">
									<header class="panel-heading font-bold">菜单信息</header>
									<div class="panel-body">
										...
										<div class="form-group">
											...
											<div class="col-sm-5">
												<!-- 这里只能加载父菜单和顶层菜单，不加载子菜单，从而限制菜单列表只能是二级的 -->
												<select name="funpid" class="form-control m-b">
													<!-- 加载出备选项 -->
													<c:forEach items="${functionList}" var="fun">
														<option value="${fun.funid}"
															<c:if test="${fun.funid == function.funid}">selected="selected"</c:if>>${fun.funname}</option>
													</c:forEach>
												</select>
											</div>
										</div>
										...
										<div class="form-group">
											<label class="col-sm-4 control-label"> 菜单名称<font
												color="red">（必填）</font>
											</label>
											<div class="col-sm-5">
												<input type="text" name="funname" class="form-control"
													data-required="true" data-rangelength="[4,15]"
													data-regexp="[\u4E00-\u9FA5\uF900-\uFA2D]+"
													data-notblank="true" value="${function.funname}"
													placeholder="请输入菜单名称...">
											</div>
										</div>
										...
										<div class="form-group">
											...
											<div class="col-sm-5">
												<input type="text" name="funurl" class="form-control"
													data-rangelength="[2,100]" data-notblank="true"
													value="${function.funurl}" placeholder="请输入菜单URL...">
											</div>
										</div>
										...
										<div class="form-group">
											...
											<div class="col-sm-5">
												<select name="funstate" class="form-control m-b">
													<option value="1"
														<c:if test="${function.funstate == 1}">selected="selected"</c:if>>正常</option>
													<option value="0"
														<c:if test="${function.funstate == 0}">selected="selected"</c:if>>禁用</option>
												</select>
											</div>
										</div>
