							...
							<section class="panel panel-default">
								...
								<form action="<%= basePath %>family/list" method="post"
									...
									<div>
										...
										</a> <a href="<%= basePath %>persinfo/list"
											class="btn btn-default"> <i class="fa  fa-chevron-left"></i>查看所有参合列表
										</a>
														...
														<c:forEach items="${page.list}" var="family">
															<tr>
																...
																<td style="line-height: 30px;text-align: center;">
																	<c:choose>
																		<c:when test="${family.perstate == 1}">已参合</c:when>
																		<c:when test="${family.perstate == 0}">未参合</c:when>
																		<c:otherwise>没有参合资格</c:otherwise>
																	</c:choose>
																</td>
																<td style="line-height: 30px;text-align: center;">
																	<a
																	href="<%= basePath %>family/toPerAdd?fid=${family.fid}"
																	class="btn btn-warning btn-sm">参合</a> <a
																	href="<%= basePath %>family/per?fid=${family.fid}"
																	class="btn btn-warning btn-sm">取消参合</a>
																</td>
															</tr>
														</c:forEach>
													</c:when>
