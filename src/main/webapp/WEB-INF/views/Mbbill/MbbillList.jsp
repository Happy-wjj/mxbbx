...
<body>
					...
					<section class="vbox">
						<section class="scrollable padder">
							...
							<ul class="breadcrumb no-border no-radius b-b b-light pull-in">
								...
								<li class="active">慢性病症管理</li>
							</ul>

							<!-- 表格标题 -->
							<div class="m-b-md">
								<h3 class="m-b-none">慢性病症列表</h3>
							</div>
							...
							<section class="panel panel-default">
								<header class="panel-heading">慢性病症管理列表</header>

								<form action="<%=basePath%>Mbbill/list" method="post"
									...
									<div class="row text-sm wrapper">
										...
										<div class="col-sm-10 m-b-xs">
											<div class="inline" style="margin-right:20px;">
												姓名 <input type="text" id="keyword" name="keyword"
											...
											<tbody>
												...
												<c:choose>
													<c:when test="${!empty page.list}">
														<c:forEach items="${page.list}" var="mbbill">
															<tr>
																<td style="line-height: 30px;text-align: center;"><%=i++%></td>
																<td style="line-height: 30px;text-align: center;">${mbbill.persname}</td>
																<td style="line-height: 30px;text-align: center;">${mbbill.cordno}</td>
																<td style="line-height: 30px;text-align: center;">${mbbill.illname}</td>
																<td style="line-height: 30px;text-align: center;">${mbbill.startday}</td>
																<td style="line-height: 30px;text-align: center;">
																	<a href="<%= basePath %>/Mbbill/toEdit?id=${mbbill.id}"
																	class="btn btn-warning btn-sm">修改</a> <a
																	href="<%= basePath %>/Mbbill/delete?id=${mbbill.id}"
	...
	<script type="text/javascript">
		...
		function getRequestURL(page) {
			...
			var url = host + "mbbill/list?pageNo=" + pageNo + "&pageCount=" + pageCount
				+ "&keyword=" + keyword;
			return url;
		}
	</script>
