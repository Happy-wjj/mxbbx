					...
					<section class="vbox">
						<section class="scrollable padder">
							...
							<section class="panel panel-default">
								...
								<form action="<%= basePath %>persinfo/list" method="post"
										...
										<table class="table table-striped b-t b-light text-sm">
											...
											<thead>
												<tr>
													...
													<th style="text-align: center;">参合时间</th>
													<th style="text-align: center;">参合证号</th>
													<!-- <th style="text-align: center;">家庭组号</th> -->
											...
											<tbody>
												...
												<c:choose>
													<c:when test="${!empty page.list}">
														<c:forEach items="${page.list}" var="persinfo">
															<tr>
																<td style="line-height: 30px;text-align: center;"><%=i++ %></td>
																<td style="line-height: 30px;text-align: center;">${persinfo.cardno}</td>
																<td style="line-height: 30px;text-align: center;">${persinfo.persname}</td>
																<td style="line-height: 30px;text-align: center;">${persinfo.sex}</td>
																<td style="line-height: 30px;text-align: center;">${persinfo.age}</td>
																<td style="line-height: 30px;text-align: center;">${persinfo.address}</td>
																<td style="line-height: 30px;text-align: center;">${persinfo.tel}</td>
																<td style="line-height: 30px;text-align: center;">${persinfo.joinyear}</td>
																<td style="line-height: 30px;text-align: center;">${persinfo.persnumber}</td>
																<%-- <td style="line-height: 30px;text-align: center;">${persinfo.family.id}</td> --%>

																<td style="line-height: 30px;text-align: center;">
																	<%-- <a href="<%= basePath %>family/toView?id=${family.id}" class="btn btn-success btn-sm">查看</a> --%>
																	<a
																	href="<%= basePath %>persinfo/toEdit?id=${persinfo.id}"
																	class="btn btn-warning btn-sm">修改</a> <a
																	href="<%= basePath %>persinfo/delete?id=${persinfo.id}"
