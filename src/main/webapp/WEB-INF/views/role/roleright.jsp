				...
				<section id="content">
					<section class="vbox">
						...
						<section class="scrollable padder">
							...
							<form id="form" action="<%= basePath %>role/updateRoleRight"
								method="post" class="form-horizontal">
								<div>
									<!-- 表单提交按钮 -->
									<a href="<%= basePath %>role/roles" class="btn btn-default"><i
										class="fa  fa-chevron-left"></i>取消</a>
									<button type="submit" class="btn btn-dark">保存权限设置</button>
								</div>
								<br /> <input type="hidden" name="roleid"
									value="${role.roleid}">

								<section class="panel panel-default">
									<header class="panel-heading font-bold"> 权限列表 </header>
												...
												<c:if test="${parent.funpid == -1 && parent.funid != -1}">
													...
													<div class="checkbox">
														<label class="checkbox-custom"> <input
															id="${parent.funid}" type="checkbox" name="funids"
															value="${parent.funid}"
														...
														<c:forEach items="${functionList}" var="child">
															<c:if test="${child.funpid == parent.funid}">
																...
																<div class="checkbox">
																	<label class="checkbox-custom"> <input
																		id="${child.funid}" type="checkbox" name="funids"
																		value="${child.funid}" parent="${child.funpid}"
																		onclick="checkBox('${child.funid}', '${child.funpid}');"
																		<%-- 回显选项 --%>
									                                        	<c:forEach items="${role.functionList}" var="fun">
									                                        		<c:if test="${child.funid == fun.funid}">checked="checked"</c:if>
									                                        	</c:forEach>>
																		<i id="i${child.funid}" class="fa fa-fw fa-square-o"></i>
																		${child.funname}
																	</label>
																</div>
															</c:if>
