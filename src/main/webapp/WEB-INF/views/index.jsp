		...
		<header class="bg-dark dk header navbar navbar-fixed-top-xs">
			...
			<div class="navbar-header aside-md">
				<a class="btn btn-link visible-xs"
					...
					src="images/logo.png" class="m-r-sm"> 新型农村合作医疗慢病报销管理信息系统
			...
			<ul class="nav navbar-nav navbar-right hidden-xs nav-user">
				<li class="dropdown"><a href="#" class="dropdown-toggle"
					...
					<ul class="dropdown-menu animated fadeInRight">
						...
						<li><a href="<%=basePath%>user/viewSelf" target="main">查看个人信息</a>
						</li>
						<li><a href="<%=basePath%>user/toModifySelf" target="main">修改个人信息</a>
						</li>
						<li><a href="<%=basePath%>user/toPasswordModify"
						...
						<section class="w-f scrollable">
							<div class="slim-scroll" data-height="auto"
								...
								<nav class="nav-primary hidden-xs">
									...
									<ul class="nav">
										<!-- 首页，默认显示这一页 -->
										<li class="active"><a href="<%=basePath%>toWelcome"
											class="active" target="main"> <i
										...
										<c:forEach items="${functionList}" var="parent">
											...
											<c:if test="${parent.funstate == 1 && parent.funpid == -1}">
												<!-- 父功能（一级菜单） -->
												<li>
													<!-- 父菜单：取消默认的点击事件行为 --> <a href="#"
													onclick="return false;"> <i
														class="fa fa-angle-right icon"> <c:choose>
																...
																<c:when test="${parent.funid % 7 == 1}">
																	<b class="bg-primary"></b>
																</c:when>
																<c:when test="${parent.funid % 7 == 2}">
																	<b class="bg-warning"></b>
																</c:when>
																<c:when test="${parent.funid % 7 == 3}">
																	<b class="bg-success"></b>
																</c:when>
																<c:when test="${parent.funid % 7 == 4}">
																	<b class="bg-dark"></b>
																</c:when>
																<c:when test="${parent.funid % 7 == 5}">
																	<b class="bg-info"></b>
																</c:when>
																<c:when test="${parent.funid % 7 == 6}">
																	<b class="bg-info"></b>
																</c:when>
																<c:when test="${parent.funid % 7 == 0}">
																	<b class="bg-info"></b>
																</c:when>
															</c:choose>
													...
													</span> <span>${parent.funname}</span>
												</a> <!-- 子菜单 -->
													<ul class="nav lt">
														<c:forEach items="${functionList}" var="child">
															<!-- 若某个父功能下面的子功能被禁用，则不加载此功能 -->
															<c:if
																test="${child.funstate == 1 && child.funpid == parent.funid}">
																<!-- 加载子菜单 -->
																<li><a href="<%= basePath %>${child.funurl}"
																	target="main"> <i class="fa fa-angle-right"></i> <span>${child.funname}</span>
																</a></li>
															</c:if>
														</c:forEach>
													</ul>
				...
				<section id="content">
					<section class="vbox">
						<section class="scrollable padder"
							...
							<iframe id="main" name="main" height="1200px" width="100%"
								scrolling="auto" frameborder="0" src="<%=basePath%>toWelcome">
